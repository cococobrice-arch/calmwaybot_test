...
async def send_material(callback: CallbackQuery):
    ...
    # Вместо немедленной отправки PDF показываем кнопку «Открыть PDF»
...
async def open_consult(callback: CallbackQuery):
    chat_id = callback.message.chat.id
    log_event(chat_id, "user_clicked_consult_cta", "Нажал кнопку «Узнать про консультации»")

    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Перейти на страницу консультаций", url="https://лечение-паники.рф/консультации")]
        ]
    )
...
async def send_final_block2(chat_id: int):
    ...
    extra_text = (
        "<b>Одно из самых частых сомнений...</b>\n\n"
        "Это абсолютно понятный вопрос...\n\n"
...
async def send_final_block3(chat_id: int):
    ...
    thoughts_text = (
        ...
        "Может складываться впечатление..."
    )
