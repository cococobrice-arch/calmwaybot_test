...
async def send_channel_invite(chat_id: int):
    ...
    try:
        ...
        if is_subscribed:
            # Фиксируем факт подписки как событие
            log_event(chat_id, "user_subscribed", "Подписан на канал к моменту проверки перед приглашением")
    ...
    # Вместо прямой URL-кнопки делаем callback, чтобы отследить клики
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Подписаться на канал", callback_data="go_to_channel")]
        ]
    )
    text = (
...
@router.callback_query(F.data == "go_to_channel")
async def cb_go_to_channel(callback: CallbackQuery):
    """Пользователь нажал кнопку перехода в канал (логируем клик и даём реальную URL-кнопку)."""
    chat_id = callback.message.chat.id
    log_event(chat_id, "user_clicked_to_channel", "Нажал кнопку перехода в канал")

    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Перейти в канал", url="https://t.me/OcdAndAnxiety")]
        ]
    )
...
async def cmd_start(message: Message):
    ...
    # 1️⃣ Полная очистка ТОЛЬКО для тестовых ID
...
# =========================================================
# 2. ОТПРАВКА ГАЙДА + КНОПКА «📘 ОТКРЫТЬ PDF»
# =========================================================
...
async def send_material(callback: CallbackQuery):
    ...
    # --- КРУЖОК --- (оставляем, как у тебя в оригинале)
    ...
    await bot.send_message(
        chat_id,
        "Ваш материал готов. Нажмите кнопку ниже, чтобы открыть PDF-гайд 👇",
        reply_markup=kb
    )

    # Планируем (как в твоей версии):
    # 1) приглашение в канал через 20 минут
    schedule_message(
        user_id=chat_id,
        prod_seconds=20 * 60,
        test_seconds=5,
    ...
    # 2) запуск опроса избегания через сутки
    schedule_message(
        user_id=chat_id,
...
async def open_pdf(callback: CallbackQuery):
    """Пользователь нажал «📘 Открыть PDF» — логируем и отправляем сам файл/ссылку."""
    chat_id = callback.message.chat.id
    await callback.answer()

    log_event(chat_id, "user_opened_pdf", "Нажал кнопку «Открыть PDF»")

    # --- локальный файл (как у тебя) ---
    ...
    # --- если PDF когда-нибудь будет по ссылке ---
    ...
    # --- ошибка ---
        ...
        await bot.send_message(
            chat_id,
            "⚠️ Файл не найден. Попробуйте позже."
        )
...
async def handle_answer(callback: CallbackQuery):
    ...
    try:
        ...
        # здесь РАНЬШЕ был log_event с деталями ответа по каждому вопросу
        # log_event(chat_id, "user_answer", f"Вопрос {idx + 1}: {ans.upper()}")
        # теперь мы не логируем отдельные ответы, чтобы в панели не светились детали теста
...
async def send_case_story(chat_id: int):
    ...
    # 11. "С людьми, переживающими..." — через сутки
    schedule_message(
        user_id=chat_id,
        prod_seconds=24 * 60 * 60,
        test_seconds=5,
...
async def send_final_message(chat_id: int):
    ...
    # Следующее сообщение через минуту
    await smart_sleep(chat_id, prod_seconds=60, test_seconds=3)

    text = (
        "По итогам прохождения психотерапии Вы получите:\n\n"
        "✨ снижение <b>гиперконтроля и проверок</b> собственного состояния\n\n"
        "✨ способность <b>снова свободно выходить из дома, ездить в метро, летать в самолётах, водить машину</b>\n\n"
        "✨ умение <b>оставаться в контакте с тревогой</b>, не убегая от неё\n\n"
        "✨ способность <b>жить спонтанно и легко</b>, не подстраиваясь под ограничения\n\n"
        "✨ внутреннюю уверенность, что <b>с Вами всё в порядке</b>\n\n"
        "Почитать подробнее о том, как проходит психотерапия со мной 👇"
    )

    # ВАЖНО: Эта кнопка должна логироваться как user_clicked_consult_cta
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Узнать про консультации", callback_data="open_consult")]
        ]
    )
...
# ---- ЛОГ CTA: Узнать про консультации ----
...
async def open_consult(callback: CallbackQuery):
    ...
    # 2. СОЗДАЁМ URL-КНОПКУ (единичную)
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
    ...
    # 3. МГНОВЕННО МЕНЯЕМ КНОПКУ В ТОМ ЖЕ СООБЩЕНИИ
    try:
        await callback.message.edit_reply_markup(reply_markup=keyboard)
    except Exception:
        pass

    # 4. Telegram сам откроет браузер после callback.answer()
    await callback.answer()



# =========================================================
# 5.2. ФИНАЛЬНЫЙ БЛОК 2
# =========================================================
async def send_final_block2(chat_id: int):
    await smart_sleep(chat_id, prod_seconds=1, test_seconds=1)
